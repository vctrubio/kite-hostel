      ...
      {roleRoutes.map(
        ({
          ...
          <Link
            key={label}
            href={href}
            className={`group relative overflow-hidden rounded-2xl border-2 p-6 transition-all duration-300 hover:scale-105 hover:shadow-2xl ${isDarkMode
          ...
          >
            ...
            <div className="relative flex flex-col items-center text-center gap-4">
              ...
              <ArrowRight
                className={`w-5 h-5 transform group-hover:translate-x-2 transition-transform duration-300 ${isDarkMode ? "text-gray-500" : "text-gray-400"
                  }`}
              />
            </div>
          </Link>
    ...
    <div className="flex flex-wrap justify-center gap-4">
      {links.map(({ icon: Icon, label, description, href, color }) => (
        <Link
          key={label}
          href={href}
          className={`group flex items-center gap-3 px-6 py-3 rounded-xl border transition-all duration-300 hover:scale-105 hover:shadow-lg ${isDarkMode
            ? "bg-gray-800/50 border-gray-700 hover:border-gray-600"
            : "bg-white/80 border-gray-200 hover:border-gray-300"
            } backdrop-blur-sm`}
        >
          ...
          <div className="text-left">
            <div
              className={`font-semibold text-sm ${isDarkMode ? "text-gray-200" : "text-gray-800"
                }`}
            >
              {label}
            </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div
          className={`absolute top-20 right-20 w-32 h-32 rounded-full blur-2xl opacity-30 animate-pulse ${isDarkMode ? "bg-cyan-500" : "bg-blue-400"
            }`}
          style={{ animationDuration: "4s" }}
        ></div>
        <div
          className={`absolute bottom-20 left-20 w-40 h-40 rounded-full blur-2xl opacity-20 animate-pulse ${isDarkMode ? "bg-green-500" : "bg-cyan-400"
            }`}
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative z-10">
        <div className="w-full max-w-5xl space-y-12">
          ...
          <div className="text-center space-y-6">
            <div className="flex justify-center mb-6">
              <button
                ...
                className={`p-6 rounded-full transition-all duration-300 hover:scale-110 cursor-pointer ${isDarkMode
              ...
              >
                <Compass
                  className={`w-20 h-20 transition-all duration-600 ${isDarkMode ? "text-cyan-400" : "text-blue-600"
                    } ${isSpinning ? "animate-spin" : ""} ${isDarkMode ? "rotate-180" : "rotate-0"
                    }`}
                />
              </button>
            </div>
            ...
            <div>
              <h1
                className={`text-8xl md:text-9xl font-bold mb-4 ${isDarkMode
              ...
              <h2
                className={`text-3xl md:text-4xl font-bold mb-4 ${isDarkMode ? "text-gray-100" : "text-gray-900"
                  }`}
              >
                Oh hi! I think you're lost
              </h2>
              <p
                className={`text-lg md:text-xl ${isDarkMode ? "text-gray-400" : "text-gray-600"
                  }`}
              >
                This page doesn't exist, but don't worry—let's get you back on
                course
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <h3
              className={`text-2xl font-bold text-center ${isDarkMode ? "text-gray-200" : "text-gray-800"
                }`}
            >
          ...
          <div className="space-y-6">
            <h3
              className={`text-xl font-semibold text-center ${isDarkMode ? "text-gray-300" : "text-gray-700"
                }`}
            >
              Or explore these
            </h3>
          ...
          <div className="text-center">
            <Link
              href="/"
              className={`inline-flex items-center gap-2 px-8 py-4 rounded-xl font-bold text-lg transition-all duration-300 hover:scale-105 hover:shadow-xl ${isDarkMode

...
export type TeacherWithRelations = InferSelectModel<typeof Teacher> & {
  ...
  bookings: (InferSelectModel<typeof Booking> & {
    ...
    reference: InferSelectModel<typeof user_wallet> & { teacher: InferSelectModel<typeof Teacher> } | null;
...
export async function getTeacherById(id: string): Promise<{ data: TeacherWithRelations | null; error: string | null }> {
  try {
    ...
    // First get all lessons for this teacher to find which bookings they're involved in
    const teacherLessons = await db.query.Lesson.findMany({
      where: eq(Lesson.teacher_id, id),
      columns: { booking_id: true },
    });

    const bookingIds = [...new Set(teacherLessons.map(lesson => lesson.booking_id))];
    ...
    // Get all bookings where this teacher has lessons, including ALL lessons in those bookings
    const bookingsWithLessons = await db.query.Booking.findMany({
    ...
    // All bookings are already filtered to contain this teacher's lessons
    const filteredBookings = bookingsWithLessons;

    // Extract only lessons from this teacher for backward compatibility
    const allLessons = filteredBookings.flatMap(booking => 
      (booking.lessons || []).filter(lesson => lesson.teacher_id === id)
    );

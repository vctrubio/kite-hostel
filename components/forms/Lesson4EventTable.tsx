...
type FilterType = "all" | "no-events" | "with-events" | "planned-only" | "active-bookings";
  ...
  return (
    <thead className="bg-muted/50">
      <tr className="border-b border-border">
        <th className="py-3 px-4 text-left text-sm font-medium text-muted-foreground">Teacher</th>
        <th className="py-3 px-4 text-left text-sm font-medium text-muted-foreground">Lesson Status</th>
        <th className="py-3 px-4 text-left text-sm font-medium text-muted-foreground">Booking Period</th>
        <th className="py-3 px-4 text-left text-sm font-medium text-muted-foreground">Students</th>
        <th className="py-3 px-4 text-left text-sm font-medium text-muted-foreground">Events</th>
...
function LessonTableRow({ lesson, expandedRow, onExpand, onAddEvent }: LessonTableRowProps) {
      ...
      <tr className="border-b border-border hover:bg-muted/20">
        <td className="py-3 px-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">{lesson.teacher?.name || "Unassigned"}</span>
          </div>
        </td>
        <td className="py-3 px-4">
          <LessonStatusLabel lessonId={lesson.id} currentStatus={lesson.status} lessonEvents={lesson.events} />
        </td>
        <td className="py-3 px-4">
          <FormatDateRange startDate={lesson.booking.date_start} endDate={lesson.booking.date_end} />
        </td>
        <td className="py-3 px-4">
          <div className="flex items-center gap-2">
            <HelmetIcon className="h-4 w-4 text-yellow-500" />
            <span className="text-sm">
              {lesson.booking.students?.map((bs: any) => bs.student.name).join(", ") || "No students"}
            </span>
          </div>
        </td>
        <td className="py-3 px-4">
          {lesson.events?.length > 0 ? (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">{lesson.events.length} event{lesson.events.length === 1 ? "" : "s"}</span>
              <span className="text-xs text-muted-foreground">(<Duration minutes={lesson.totalKiteEventDuration} />)</span>
            </div>
          ) : (
        ...
        <td className="py-3 px-4">
          <div className="flex items-center justify-center gap-1">
            {lesson.status === "planned" && lesson.booking.status === "active" && (
              <Button variant="outline" size="sm" onClick={() => onAddEvent(lesson)} className="text-xs">
                <Plus className="h-3 w-3 mr-1" />
                Add Event
              </Button>
            )}
            <Button variant="ghost" size="sm" onClick={() => onExpand(lesson.id)}>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 p-3 bg-background/50 rounded-md border-l-4 border-blue-500">
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  <BookingStatusLabel bookingId={lesson.booking.id} currentStatus={lesson.booking.status} />
                  <span className="text-sm text-muted-foreground">Ref: {getUserWalletName(lesson.booking.reference) || "No reference"}</span>
                ...
                <div className="p-3 bg-background/50 rounded-md border-l-4 border-yellow-500">
                  <div className="flex flex-wrap gap-2">
                    {lesson.booking.students.map((bs: any) => (
                      <div key={bs.student.id} className="flex items-center gap-1 px-2 py-1 bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300 rounded text-sm font-medium">
              ...
              {lesson.events?.length > 0 ? (
                <div className="p-3 bg-background/50 rounded-md border-l-4 border-teal-500">
                  ...
                  <div className="mt-3 pt-3 border-t border-border">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">Total event time:</span>
                      <span className="font-medium">
                        <Duration minutes={lesson.totalKiteEventDuration} /> of <Duration minutes={lesson.packageDuration} />
                      </span>
                    </div>
                  </div>
                  {lesson.status === "planned" && lesson.booking.status === "active" && (
                    <div className="mt-3 pt-3 border-t border-border">
                      <Button variant="outline" size="sm" onClick={() => onAddEvent(lesson)} className="text-xs w-full">
                        <Plus className="h-3 w-3 mr-1" />
                        Add Another Event
                      </Button>
                    </div>
                ...
                <div className="p-3 bg-background/50 rounded-md border-l-4 border-gray-500">
                  <div className="text-center">
                    <p className="text-sm text-muted-foreground mb-3">No events scheduled for this lesson</p>
                    {lesson.status === "planned" && lesson.booking.status === "active" && (
                      <Button variant="outline" size="sm" onClick={() => onAddEvent(lesson)} className="text-xs">
                        <Plus className="h-3 w-3 mr-1" />
                        Create First Event
...
function LessonTable({ lessons, expandedRow, onExpand, onAddEvent }: LessonTableProps) {
  return (
      ...
      <table className="w-full">
        ...
        <tbody>
          {lessons.length === 0 ? (
            ...
            lessons.map((lesson) => (
              <LessonTableRow key={lesson.id} lesson={lesson} expandedRow={expandedRow} onExpand={onExpand} onAddEvent={onAddEvent} />
            ))
          )}
        </tbody>
      </table>
...
export function Lesson4EventTable({ lessons, onRefresh }: Lesson4EventTableProps) {
  ...
  const filteredLessons = useMemo(() => {
    switch (filter) {
      case "no-events":
        return lessons.filter((lesson) => !lesson.events || lesson.events.length === 0);
      case "with-events":
        return lessons.filter((lesson) => lesson.events && lesson.events.length > 0);
      case "planned-only":
  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-sm">
      ...
      />
      <LessonTable lessons={filteredLessons} expandedRow={expandedRow} onExpand={toggleExpand} onAddEvent={handleAddEvent} />
      {isModalOpen && selectedLesson && (
        <EventToTeacherModal
          ...
          lesson={{
            ...selectedLesson,
            students: selectedLesson.booking.students?.map(bs => bs.student) || [],
          ...
          remainingMinutes={selectedLesson.packageDuration - selectedLesson.totalKiteEventDuration}
          allowDateEdit={true}
        />
      )}
    </div>

...
export default function TeacherEventQueue({
  ...
  const handleRemoveGapForScheduleNode = (lessonId: string) => {
    ...
    const nodeWithGap = nodesWithGaps.find(
  ...
  return (
    <>
      {scheduleNodes.map((node, _index) => {
        ...
        const studentList = eventData.booking
        ...
        const timeAdjustment = originalNode
          ...
          timeToMinutes(originalNode.startTime)
          : 0;
        ...
        if (teacherSchedule) {
          ...
          const nodesWithGaps =
            teacherSchedule.analyzeScheduleGaps(scheduleNodes);

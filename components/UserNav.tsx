...
function UserNavRoutes() {
  ...
  const formRoutes = ['/students', '/teachers', '/packages', '/bookings', '/lessons', '/events', '/kites', '/payments', '/references'];
  ...
  const getCurrentEntityInfo = () => {
    if (pathname.startsWith('/students')) return { label: 'Students', icon: 'HelmetIcon', color: 'text-yellow-500' };
    if (pathname.startsWith('/teachers')) return { label: 'Teachers', icon: 'HeadsetIcon', color: 'text-green-500' };
    if (pathname.startsWith('/packages')) return { label: 'Packages', icon: 'BookmarkIcon', color: 'text-orange-500' };
    if (pathname.startsWith('/bookings')) return { label: 'Bookings', icon: 'BookingIcon', color: 'text-blue-500' };
    if (pathname.startsWith('/lessons')) return { label: 'Lessons', icon: 'FlagIcon', color: 'text-cyan-500' };
    if (pathname.startsWith('/events')) return { label: 'Events', icon: 'KiteIcon', color: 'text-teal-500' };
    if (pathname.startsWith('/kites')) return { label: 'Kites', icon: 'EquipmentIcon', color: 'text-purple-500' };
    if (pathname.startsWith('/payments')) return { label: 'Payments', icon: 'PaymentIcon', color: 'text-amber-500' };
    if (pathname.startsWith('/references')) return { label: 'References', icon: 'BookIcon', color: 'text-slate-400' };
  ...
  const routes = [
    ...
    { 
      ...
      icon: isOnFormRoute && entityInfo.icon !== 'FileText' ? 
        (entityInfo.icon === 'HelmetIcon' ? HelmetIcon :
         entityInfo.icon === 'HeadsetIcon' ? HeadsetIcon :
         entityInfo.icon === 'BookmarkIcon' ? BookmarkIcon :
         entityInfo.icon === 'BookingIcon' ? BookingIcon :
         entityInfo.icon === 'FlagIcon' ? FlagIcon :
         entityInfo.icon === 'KiteIcon' ? KiteIcon :
         entityInfo.icon === 'EquipmentIcon' ? EquipmentIcon :
         entityInfo.icon === 'PaymentIcon' ? PaymentIcon :
         entityInfo.icon === 'BookIcon' ? BookIcon : FileText) : FileText,
      color: isOnFormRoute ? entityInfo.color : "text-gray-500",
      hoverColor: isOnFormRoute ? `hover:opacity-80 hover:bg-gray-50` : "hover:text-gray-700 hover:bg-gray-100"
    },
    { 
      href: "/users", 
      label: "Users", 
  ...
  return (
    <div className="flex items-center space-x-3">
      {routes.map(({ href, label, icon: Icon, color, hoverColor, hasQuickAction, quickActionHref, quickActionIcon: QuickActionIcon }) => {
          ...
          return (
            <div key={href} className="relative">
              <div className="flex items-center">
                ...
                >
                  <Icon className={`h-4 w-4 ${isOnFormRoute ? entityInfo.color : ''}`} />
                  <span>{label}</span>
                </Link>
                      ...
                      return (
                        <div key={entity.name} className="flex items-center">
                          <Link
                            ...
                            style={{ '--hover-bg': getHoverColor() } as React.CSSProperties}
        ...
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <div className={`text-xs text-muted-foreground transition-opacity duration-300 min-h-[12px] ${!note && fetchingWallet ? "bg-muted rounded animate-pulse w-16" : ""}`}>
                {note && <span>{note}</span>}
              </div>
              <ThemeSwitcher />
              <div className={`transition-opacity duration-300 ${!user ? "opacity-50" : "opacity-100"}`}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1">
                <div className={`font-semibold transition-opacity duration-300 min-h-[18px] ${!displayName ? "bg-muted rounded animate-pulse" : ""}`}>
                  {displayName || <span className="invisible">Loading name</span>}
                </div>
                <div className={`text-xs text-muted-foreground transition-opacity duration-300 min-h-[14px] ${!role ? "bg-muted rounded animate-pulse mt-1" : ""}`}>
                  {role || <span className="invisible">guest</span>}
                </div>
                {email && (
                  <div className={`text-xs text-muted-foreground transition-opacity duration-300 min-h-[14px] ${!email ? "bg-muted rounded animate-pulse mt-1" : ""}`}>
                    ({email})
                  </div>
                )}
              </div>

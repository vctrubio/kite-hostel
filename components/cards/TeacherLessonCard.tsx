...
export default function TeacherLessonCard({ lessonDetail, commission }: TeacherLessonCardProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  
  const completedHours = lessonDetail.events
    .filter(event => event.status === 'completed')
    .reduce((sum, event) => sum + event.duration, 0) / 60;
  
  const totalEventDuration = lessonDetail.events
    ...
    <div className="bg-card border border-border rounded-lg overflow-hidden">
      ...
      <div className="p-4 bg-gray-50 dark:bg-gray-800 border-b border-border">
        <div className="flex justify-between items-start mb-2">
          <div>
            <h3 className="font-medium text-lg">{lessonDetail.packageInfo.description || "Lesson Package"}</h3>
            <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Duration: <Duration minutes={lessonDetail.packageInfo.duration} /> • 
              Kite Capacity: {lessonDetail.packageInfo.capacity_kites} kites
            </div>
          </div>
          <div className="text-right">
            <div className="text-lg font-bold text-green-600">€{Math.round(earnings)}</div>
            <div className="text-xs text-gray-500">{completedHours.toFixed(1)}h × €{commissionRate}/h</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <DateTime dateString={lessonDetail.lesson.booking.date_start} formatType="date" /> - 
          <DateTime dateString={lessonDetail.lesson.booking.date_end} formatType="date" />
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        >
          <span className="font-medium">Events ({lessonDetail.events.length}) • <Duration minutes={totalEventDuration} /></span>
              ...
              <div key={event.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                ...
                <div className="flex items-center gap-4 text-sm flex-1">
                  ...
                  <span className={`px-2 py-1 text-xs rounded-full ${
                    event.status === 'completed' ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' :
                    event.status === 'planned' ? 'bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100' :
                    event.status === 'tbc' ? 'bg-purple-100 text-purple-800 dark:bg-purple-800 dark:text-purple-100' :
                  ...
                  {event.kites.length > 0 && (
                    <span className="text-gray-600 dark:text-gray-400">
                      {event.kites.map(k => `${k.kite.model} ${k.kite.size}m`).join(", ")}
                    </span>
                  )}
                </div>

...
CREATE TYPE "public"."kite_event_status" AS ENUM('active', 'completed', 'tbc');--> statement-breakpoint
CREATE TYPE "public"."languages" AS ENUM('Spanish', 'French', 'English', 'German', 'Italian');--> statement-breakpoint
CREATE TYPE "public"."lesson_status" AS ENUM('active', 'rest', 'delegated', 'completed', 'cancelled');--> statement-breakpoint
CREATE TYPE "public"."location" AS ENUM('Los Lances', 'Valdevaqueros', 'Palmones');--> statement-breakpoint
CREATE TYPE "public"."user_role" AS ENUM('guest', 'teacher', 'admin', 'teacherAdmin');--> statement-breakpoint
...
CREATE TABLE "kite_event_equipment" (
	...
	CONSTRAINT "kite_event_equipment_unique" UNIQUE("kite_event_id","kite_id")
);
--> statement-breakpoint
CREATE TABLE "lesson" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE "teacher" (
	...
	"country" text,
	"phone" text,
	"commission_a" integer NOT NULL,
	"commission_b" integer,
	"created_at" timestamp DEFAULT now(),
	"deleted_at" timestamp,
...
ALTER TABLE "kite_event_equipment" ADD CONSTRAINT "kite_event_equipment_kite_id_fk" FOREIGN KEY ("kite_id") REFERENCES "public"."kite"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "kite_event_equipment_kite_id_idx" ON "kite_event_equipment" USING btree ("kite_id");--> statement-breakpoint
